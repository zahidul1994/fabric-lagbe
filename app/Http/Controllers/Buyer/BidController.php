...
//        $product_bid = ProductBid::find($id);
//        $seller = User::where('id',$product_bid->receiver_user_id)->first();
//        return view('frontend.buyer.product_bids.chat_with_admin',compact('product_bid','seller'));
//    }
    public function chatWithAdmin($id){
        $saleRecord = SaleRecord::where('product_bid_id',$id)->first();
        $buyer = User::where('id',$saleRecord->buyer_user_id)->first();
        return view('frontend.buyer.product_bids.chat_with_admin',compact('saleRecord','buyer'));
    }
