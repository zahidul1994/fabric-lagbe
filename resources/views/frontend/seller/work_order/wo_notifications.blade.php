                                    ...
                                    <table id="example1" class="table table-bordered table-striped">
                                        ...
                                        @foreach($notifications as $key => $notification)
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="btn-new btn btn-{{$notification->receiver_view_status == 0 ? 'info' : 'success'}}">{{$notification->receiver_view_status == 0 ? 'Unseen' : 'Seen'}}</span>
                                                    @if($notification->title == 'Request for Quotation')
                                                        <a class="btn-new btn btn-warning mt-2 p-0" href="{{route('seller.my-work-order.quotation-list',encrypt($notification->work_order_product_id))}}">Bidder List</a>
                                                    @endif
                                                </td>
                                                <td>
                                                    <a class="btn-new btn btn-success" href="{{route('seller.work-order.notification.detail',encrypt($notification->id))}}">
