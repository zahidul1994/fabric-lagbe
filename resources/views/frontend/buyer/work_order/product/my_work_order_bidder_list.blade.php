...
@section("title","My Work Order Bidder list")
@push('css')
                        ...
                        <div class="col-12">
                            <div class="card card-info card-outline">
                                <div class="card-header">
                                    <h3 class="card-title float-left">Bidder List</h3>
                                </div>
                                <!-- /.card-header -->
                                    ...
                                    <table id="example1" class="table table-bordered table-striped">
                                        ...
                                        @foreach($wo_bids as $key => $wo_bid)
                                            ...
                                            <tr>
                                                ...
                                                <td>{{$wo_bid->sender->address}}</td>
                                                <td>{{two_digit_single_price($wo_bid->total_price)}}</td>
                                                <td>{{date('j M Y h:i A',strtotime($wo_bid->created_at))}}</td>
                                                @php
                                                $status = \App\Model\WorkOrderBid::where('work_order_product_id',$wo_bid->work_order_product_id)->where('bid_status',1)->first();
                                                @endphp
                                                <td>
                                                    @if($wo_bid->bid_status == 1)
